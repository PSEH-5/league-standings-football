...
public class LeagueApplicationTests {
	...
	{
		try {
			List<FootBallLeagueResponse>   response=standingService.getFootBallStandingsData("France", 2, 7);
	...
	{
		try {
			List<FootBallLeagueResponse>   response=standingService.getFootBallStandingsData("Scotland123", 5, 8);
			assert(response.isEmpty());
		}
		catch (Exception e) {
			assert(true);
